...
class XmlrpcSource extends DataSource
{
    ...
    protected function _request($method, $params)
    {
        ...
        if (!$this->HttpSocket) {
            $this->HttpSocket = new HttpSocket(['timeout' => $this->config['timeout']]);
        }
        $uri = [
        ...
        try {
            $response = $this->HttpSocket->post($uri, $xmlRequest, ['header' => ['Content-Type' => 'text/xml']]);
        } catch (Exception $e) {
     ...
     */
    public function generateXML($method, $params = [])
    {
        $query = [
            'methodCall' => [
                'methodName' => $method,
        ...
        $Xml = Xml::fromArray($query, ['format' => 'tags']);
    ...
    protected function _normalizeParam($param)
    {
        if (is_array($param)) {
            ...
            return ['value' => ['struct' => ['member' => $members]]];
        }
        if (is_int($param)) {
                ...
                if (isset($value['data']['value']) && is_array($value['data']['value'])) {
                    foreach ($value['data']['value'] as $key => $newValue) {
                        ...
                        if ($key === 'struct' || $key === 'array') {
                            $return[] = $this->_parseResponse([$key => $newValue]);
                        } else {

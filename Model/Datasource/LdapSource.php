...
class LdapSource extends DataSource
{
    ...
    protected function _deleteRecursively($dn)
    {
        ...
        $subentries = ldap_list($this->database, $dn, 'objectClass=*', []);
     ...
     * @param string $association *unused*
    ...
    public function queryAssociation(Model $model, &$linkModel, $type, $association, $assocData, &$queryData, $external, &$resultSet, $recursive, $stack)
    {
        if (!isset($resultSet) || !is_array($resultSet)) {
            if (Configure::read('debug') > 0) {
                echo '<div style = "font-family: Verdana; font-weight:  bold; font-size: 12px; color: #FF0000">SQL Error in model ' . $model->name . ': ';
    ...
    {
        $schemaTypes = ['objectclasses', 'attributetypes'];
    ...
    {
        $opsRec = ['and' => ['prefix' => '&'], 'or' => ['prefix' => '|']];
        $opsNeg = ['and not' => [], 'or not' => [], 'not equals' => []];
        $opsTer = ['equals' => ['null' => '*']];
        ...
        if (is_array($conditions)) {
            ...
            if (in_array($operand, array_keys($opsNeg))) {
                ...
                return '(!' . $this->_conditionsArrayToString([$nextOperand => $children]) . ')';
            }
            ...
            if (in_array($operand, array_keys($opsTer))) {
                ...
                foreach ($children as $key => $value) {
                    if (!is_array($value)) {
                        $tmp .= '(' . $key . '=' . ($value ?? $opsTer['equals']['null']) . ')';
                    } else {
                        foreach ($value as $subvalue) {
                            $tmp .= $this->_conditionsArrayToString(['equals' => [$key => $subvalue]]);
                        }
                    }
                }
     ...
     */
    protected function _executeQuery($queryData = [], $cache = true)
    {
        $t = microtime(true);
    ...
    {
        $opts = ['LDAP_OPT_DEREF', 'LDAP_OPT_SIZELIMIT', 'LDAP_OPT_TIMELIMIT', 'LDAP_OPT_NETWORK_TIMEOUT',
    ...
    {
        $checkDN = ldap_read($this->database, '', 'objectClass=*', ['subschemaSubentry']);

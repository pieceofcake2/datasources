...
class DboSybase extends DboSource
{
     ...
     */
    public $columns = [
        'primary_key' => ['name' => 'numeric(9,0) IDENTITY PRIMARY KEY'],
    ...
    public function column($real)
    {
        ...
        if (in_array($col, ['datetime', 'smalldatetime'])) {
            return 'datetime';
        }
        if (in_array($col, ['int', 'bigint', 'smallint', 'tinyint'])) {
            return 'integer';
        }
        if (in_array($col, ['float', 'double', 'real', 'decimal', 'money', 'numeric', 'smallmoney'])) {
            return 'float';
        }
        if (strpos($col, 'text') !== false) {
            return 'text';
        }
        if (in_array($col, ['char', 'nchar', 'nvarchar', 'string', 'varchar'])) {
            return 'binary';
        }
        if (in_array($col, ['binary', 'image', 'varbinary'])) {
            return 'binary';
        }

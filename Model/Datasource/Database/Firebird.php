...
class DboFirebird extends DboSource
{
    ...
    public $columns = [
        ...
        'text' => ['name' => 'BLOB SUB_TYPE 1 SEGMENT SIZE 100 CHARACTER SET NONE'],
        'integer' => ['name' => 'integer'],
        'float' => ['name' => 'float', 'formatter' => 'floatval'],
        'datetime' => ['name' => 'timestamp', 'format' => 'd.m.Y H:i:s', 'formatter' => 'date'],
        'timestamp' => ['name' => 'timestamp', 'format' => 'd.m.Y H:i:s', 'formatter' => 'date'],
        'time' => ['name' => 'time', 'format' => 'H:i:s', 'formatter' => 'date'],
        'date' => ['name' => 'date', 'format' => 'd.m.Y', 'formatter' => 'date'],
        'binary' => ['name' => 'blob'],
    ...
    public function describe($model)
    {
        ...
        for ($i = 0; $i < $coln; $i++) {
            ...
            $fields[strtolower($colInfo['name'])] = [
     ...
     * @return array
     */
    public function update(Model $model, $fields = [], $values = [])
    {
    ...
    public function column($real)
    {
        ...
        if (in_array($col, ['FLOAT', 'NUMERIC', 'DECIMAL'])) {
            return 'float';
        }
    ...
    public function resultSet(&$results)
    {
        ...
        while ($j < $numFields) {
            ...
            if (!empty($column[2])) {
                $this->map[$index++] = [ucfirst(strtolower($this->modeltmp[strtolower($column[2])])), strtolower($column[1])];
            } else {
                $this->map[$index++] = [0, strtolower($column[1])];
            }
            $j++;
        }
    }
    ...
    public function renderStatement($type, $data)
    {
        ...
        if (strtolower($type) === 'select') {
            if (preg_match('/offset\s+([0-9]+)/i', $limit, $offset)) {
                ...
                [$order2, $rOrder] = [$this->_mapFields($order), $this->_mapFields($rOrder)];

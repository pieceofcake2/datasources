...
class ArraySource extends DataSource
{
    ...
    public function describe($model)
    {
        if (!empty($model->records[0])) {
            return array_fill_keys(array_keys($model->records[0]), []);
        }
    ...
    public function conditionsFilter(Model $model, $record, $conditions, $or = false)
    {
        foreach ($conditions as $field => $value) {
            ...
            if (is_array($value) && in_array(strtoupper($field), ['AND', 'NOT', 'OR'])) {
    ...
    {
        $assocData = array_merge(['conditions' => null, 'fields' => null, 'order' => null], $assocData);
        ...
        foreach ($resultSet as &$record) {
            ...
            if ($type === 'belongsTo') {
                if (isset($record[$model->alias][$assocData['foreignKey']])) {
                    $conditions = array_merge($query['conditions'], [$linkModel->alias . '.' . $linkModel->primaryKey => $record[$model->alias][$assocData['foreignKey']]]);
                    $limit = 1;
                    $data = $this->read($linkModel, compact('conditions', 'limit') + $query);
                }
            } elseif (($type === 'hasMany' || $type === 'hasOne') && $model->recursive > 0) {
                $conditions = array_merge($query['conditions'], [$linkModel->alias . '.' . $assocData['foreignKey'] => $record[$model->alias][$model->primaryKey]]);
                ...
                $fields = [$joinModel->alias . '.' . $assocData['associationForeignKey']];
                $conditions = [$joinModel->alias . '.' . $assocData['foreignKey'] => $record[$model->alias][$model->primaryKey]];
                $recursive = -1;
                ...
                if ($ids) {
                    ...
                    $conditions = array_merge($query['conditions'], [$linkModel->alias . '.' . $linkModel->primaryKey => $ids]);
            ...
            if ($type === 'hasOne' || $type === 'belongsTo') {
                $record += [$linkModel->alias => $formatted[0][$linkModel->alias]];
                continue;
            }
            $record += [$linkModel->alias => Hash::extract($formatted, "{n}.{$linkModel->alias}")];
        }
    }
    ...
    public function getLog($sorted = false, $clear = true)
    {
        ...
        return ['log' => $log, 'count' => count($log), 'time' => array_sum(Hash::extract($log, '{n}.took'))];
    }
